...
func ValidateTurnstile(ctx context.Context, token string, remoteIP string, secretKey string) error {
	if secretKey == "" {
		log.Printf("warning: Turnstile secretKey is empty, skipping validation tokenLen=%d remoteIP=%q", len(token), remoteIP)
		return nil
	}
	if remoteIP == "" {
		log.Printf("warning: Turnstile remoteIP is empty, skipping validation tokenLen=%d", len(token))
		return nil
	}
	if token == "" {
